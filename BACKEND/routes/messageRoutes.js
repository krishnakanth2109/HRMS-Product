...
// io lives in server.js → app.set("io", io)
// Make sure your server.js has Socket.IO set up and app.set("io", io)
...
router.post("/send", protect, async (req, res) => {
  try {
    ...
    // ✅ FIX: was req.user.employeeId (a string like "ARA03") — that gets saved
    //         as sender, but receiver is an ObjectId, so history queries using
    //         $or [{sender: me, receiver: X}, {sender: X, receiver: me}]
    //         never match both sides → messages vanish from history.
    //         Always use req.user._id (MongoDB ObjectId) for both sender/receiver.
    ...
    if (groupId) {
      ...
      const senderInGroup =
        group.members.some(m => m.employee.toString() === senderId.toString()) ||
        group.groupLeader.toString() === senderId.toString();
      const receiverInGroup =
        group.members.some(m => m.employee.toString() === receiverId) ||
        group.groupLeader.toString() === receiverId;
...
// ✅ FIX: frontend calls /history/:id — old backend only had /conversation/:id
// ─────────────────────────────────────────────────────────────────────────────
router.get("/history/:userId", protect, async (req, res) => {
  try {
    const { userId } = req.params;
    const me = req.user._id; // ✅ ObjectId, consistent with how sender is saved
...
// ✅ FIX: frontend calls /users — old backend only had /inbox
// ─────────────────────────────────────────────────────────────────────────────
...
// Keep old /inbox and /conversation/:userId as aliases so nothing else breaks
...
router.get("/conversation/:userId", protect, async (req, res) => {
  ...
  // Redirect internally to /history/:userId handler
  try {
    const { userId } = req.params;
    const me = req.user._id;
    const messages = await Message.find({
      ...
      .populate("receiver", "name employeeId")
      .sort({ createdAt: 1 });
    Message.updateMany({ sender: userId, receiver: me, isRead: false }, { isRead: true }).catch(console.error);
    return res.json(messages);
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
