...
const EmployeeProfile = () => {
  ...
  useEffect(() => {
    const fetchOnboardingDetails = async () => {
      ...
      try {
        // Calling verify-email to get the invitation/onboarding record
        const response = await api.post('/api/invited-employees/verify-email', { 
          email: employee.email 
        });

        if (response.data.success || response.data.onboarded) {
          setOnboardingData(response.data.data);
        }
      } catch (error) {
        const errorData = error.response?.data;
        // The API returns 400 if already onboarded, but includes the data object
        if (errorData?.data) {
            setOnboardingData(errorData.data);
        } else {
                  ...
                  <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[45%] p-4 rounded-xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-bold text-gray-900">{safe(exp.company)}</h4>
