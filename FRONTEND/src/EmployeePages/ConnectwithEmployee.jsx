...
const POLL_INTERVAL = 3000; // silent background poll every 3s (production fallback)

// ✅ Safe ObjectId comparison — fixes === failing between ObjectId object and string
...
const ConnectWithEmployee = () => {
  ...
  // ✅ Live refs so socket listeners never close over stale state
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // SILENT BACKGROUND POLLING — runs every 3s as production fallback
  // Never touches loading states → UI never blinks or flashes
  // ─────────────────────────────────────────────────────────────────────────
  const silentPollMessages = useCallback(async () => {
    ...
    try {
      ...
      const currentIds = new Set(directMessagesRef.current.map(m => m._id));
      const hasNew = fresh.some(m => !currentIds.has(m._id) && !m._id?.toString().startsWith("temp_"));

      if (hasNew) {
        setDirectMessages(prev => {
          const temps = prev.filter(m => m._id?.toString().startsWith("temp_"));
          const nonTemps = prev.filter(m => !m._id?.toString().startsWith("temp_"));
          const existingIds = new Set(nonTemps.map(m => m._id));
          const toAdd = fresh.filter(m => !existingIds.has(m._id) && !m._id?.toString().startsWith("temp_"));
          return [...nonTemps, ...toAdd, ...temps];
        });
  ...
  const silentPollChatList = useCallback(async () => {
    try {
      ...
      const hasChanges = data.some(d => {
        ...
        return !existing || existing.lastMessage !== d.lastMessage || existing.unreadCount !== d.unreadCount;
      });

      if (hasChanges) {
        setChatList(data.map(d => {
          const activePeer = selectedChatUserRef.current;
          if (activePeer && sameId(d._id, activePeer._id)) return { ...d, unreadCount: 0 };
          return d;
        }));
  ...
  // Restart poll on chat switch so new messages load quickly
  useEffect(() => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // SOCKET INIT
  // ✅ polling FIRST for Render.com production compatibility
  // ─────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!currentUserId) return;
    ...
    // ✅ Instant delivery via socket (polling catches it anyway at next tick)
  ...
  const displayList = useMemo(() => {
    ...
    if (term) {
      // Search mode — show ALL employees so user can start new chats
      return employees
        .filter(e =>
        ...
        .map(emp => {
          const history = chatList.find(c => sameId(c._id, emp._id));
          return history ? { ...emp, ...history } : emp;
        })
        .sort((a, b) => (a.name || "").localeCompare(b.name || ""));
    }

    // Default — only show employees with chat history, sorted by most recent
    return [...chatList].sort((a, b) =>
      new Date(b.lastMessageTime || 0) - new Date(a.lastMessageTime || 0)
    );
  }, [directSearchTerm, chatList, employees]);

  // ── Data fetchers ────────────────────────────────────────────────────────
  const fetchChatList = useCallback(async () => {
    try {
      // ✅ FIX: was /inbox — now /users to match route
      const { data } = await api.get("/api/chat/users");
      setChatList(data || []);
  ...
  const fetchDirectMessages = useCallback(async (peer, silent = true) => {
    ...
    try {
      // ✅ FIX: was /conversation/:id — now /history/:id to match route
  ...
  const handleDeleteMessage = useCallback(async (msgId) => {
    ...
    setOpenMenuId(null);
    try { await api.delete(`/api/chat/${msgId}`); toast.success("Message deleted"); }
  ...
  return (
    <div className="flex w-full h-[calc(100vh-4rem)] bg-white font-sans mt-0">
      <Toaster
        position="top-right"
        toastOptions={{
          style: { background: '#363636', color: '#fff', fontSize: '12px', padding: '8px 16px' },
        }}
      />
              ...
              displayList.map(emp => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 px-3 py-3 rounded-2xl cursor-pointer mb-1.5 transition-all duration-200 ${isActive
                        ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-200"
                        : "hover:bg-gray-50 hover:shadow-md"
                      } ${isSwitchingUser ? "pointer-events-none opacity-70" : ""}`}
                  >
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-base flex-shrink-0 ${isActive
                    ...
                    <div className="flex-1 overflow-hidden min-w-0">
                      <div className="flex justify-between items-center mb-0.5">
                        ...
                        <span
                          className={`text-xs font-medium flex-shrink-0 ml-1 ${isActive ? "text-blue-100" : "text-gray-500"
                            }`}
                        >
                          {emp.lastMessageTime ? (
                            new Date(emp.lastMessageTime).toLocaleTimeString([], {
                              hour: "2-digit",
                              minute: "2-digit",
                            })
                          ) : (
                ...
                <div className="p-6">
                  {Object.entries(groupedMessages).map(([date, msgs]) => (
                    <div key={date}>
                      ...
                      {msgs.map((msg, index) => {
                        // ✅ FIX: sameId() not === — ObjectId vs string comparison
            ...
            <footer className="p-4 bg-white border-t border-gray-200 shadow-lg flex-shrink-0">
              ...
              <div className="max-w-4xl mx-auto flex items-end gap-3 bg-gray-100 border border-transparent rounded-2xl p-3 focus-within:bg-white focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-500/10 transition-all duration-300">
                ...
                <button
                  ...
                  className={`p-3.5 rounded-xl transition-all shadow-md flex items-center justify-center ${directMsgText.trim() && !sendingMessage && !isSwitchingUser && !isChatLoading
